...
void vector::reserve(int new_space)
{
  ...
  delete[] elem;      // destroy old elements and
...
Note that `std::allocator` member functions `construct()` and `destroy()` were deprecated in C++17. Instead, the following *uninitialized memory algorithms* are available in `<memory>` since C++17:
...
Since C++20, the type parameter `T` of [`std::swap()`](https://en.cppreference.com/w/cpp/algorithm/swap) must meet the requirements of `MoveConstructible` and `MoveAssignable`. Therefore, `vector_base` must provide the move constructor and the move assignment operator.
...
With `vector_base`, we can implement `vector`:
