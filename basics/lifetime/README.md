...
- does not change any properties of `xB`; its value (`'B'`) and memory-address remain unchanged (`xBaddr == xB.addr()`).
...
Member-wise copy is also known as *shallow copy*. It is easy to define such simple a copy assignment operator.
...
You might notice that when calling the function `ident()`, the preceding code made copy operations three times:
