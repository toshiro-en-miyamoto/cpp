// g++ item01_ut.cpp -std=c++20
#include <typeinfo>
#include <utility>
#include <boost/ut.hpp>

// these five template functions help cheat how our compiler
...
template<typename T> type_pair ff_cref(const T& param)
{ return {typeid(T), typeid(param)}; }
  ...
  "case1b"_test = [] {
    given("non-const argument") = [] {
      when("calling f() taking const T&") = [] {
        int x = 27;
        auto [type_arg, param_arg] = ff_cref(x);
    ...
    given("const argument") = [] {
      when("calling f() taking const T&") = [] {
        int x = 27;
        const int cx = x;
        auto [type_arg, param_arg] = ff_cref(cx);
    ...
    given("const ref argument") = [] {
      when("calling f() taking const T&") = [] {
        int x = 27;
        const int& rx = x;
        auto [type_arg, param_arg] = ff_cref(rx);
