...
If `sp1` and `sp2` are `std::shared_ptr`s to different objects, the assignment `sp1 = sp2;` modifies `sp1` such that it points to the object pointed to by `sp2`. The net effect of the assignment is:
...
For `std::unique_ptr`, the type of the `delete`r is part of the type of the smart pointer. For `std::shared_ptr`, it’s not:
...
- A control block is created when a `std::shared_ptr` is constructed from unique-ownership pointer.
